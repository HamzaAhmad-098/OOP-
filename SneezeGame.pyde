# --- Sneeze Attack! Menu and Level Selection System ---
add_library('sound')
...
def setup():
    ...
    try:
        ...
        sound_timeup = SoundFile(this, "timeUp.wav")
...
def handle_timer_gameover():
    ...
    if timer_gameover:
        if sound_timeup and not hasattr(handle_timer_gameover, "played"):
            sound_timeup.play()
            handle_timer_gameover.played = True
        fill(255, 0, 0, 220)
        rectMode(CENTER)
        ...
        if hasattr(handle_timer_gameover, "played"):
            del handle_timer_gameover.played
